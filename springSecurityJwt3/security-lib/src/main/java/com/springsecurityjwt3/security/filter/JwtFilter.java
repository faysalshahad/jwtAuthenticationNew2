package com.springsecurityjwt3.security.filter;
...
import com.springsecurityjwt3.security.config.ServerMetadata;
...
public class JwtFilter extends OncePerRequestFilter {
    ...
    // this.userEntityService.loadUserByUsername(username);

    // // Fetch the entity to check the logout timestamp
    // UserEntity userEntity =
    // userEntityRepository.findByUsername(username).orElse(null);
    ...
    // serverMetadata.getServerStartTime());

    // // --- SERVER RESTART CHECK ---
    // // If the token was issued before the current server instance started, reject
    // it.
    // if(issuedAtLDT.isBefore(serverMetadata.getServerStartTime())){
    // response.sendError(HttpServletResponse.SC_UNAUTHORIZED, "Session expired due
    // to server restart. Please login again.");
    // return;
    // }
    ...
    // response.sendError(HttpServletResponse.SC_UNAUTHORIZED, "Token has been
    // revoked.");
